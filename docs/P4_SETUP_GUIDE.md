...
**パワーアップ版設定**
- **BB_Overshoot_Percent**: 0.5（BB超過50%で検出）
- **DetectBBOvershoot**: true（BB超過検出有効）
- **Use75Percent_Entry**: true（75%/25%エントリー構成）
- **ReducedRRRatio**: 0.75（BB超過時のRR 0.75:1に縮小）
...
## 【NEW】P4手法パワーアップ版 - BB超過対応

### BBオーバーシュート検出

**問題**: パーフェクトオーダー期間でも以下の場合にエントリーが難しい
- パーフェクトオーダーが崩れやすい時期
- 交差が激しい期間
- エントリー直後に反転する

**解決策**: ボリンジャーバンド（BB）を活用した2つの対応

#### ① BB超過時の対応
```
価格がBB（±50%超過）を大きくはみ出た直後のエントリー
↓
リスクが高い（騙し反転の可能性大）
↓
RRを1:0.75～1:1に自動縮小
...
**例**
```
Signal 1, 2, 3, 4, 5: 通常エントリー
Signal 6, 7, 8: 通常エントリー
Signal 9, 10, 11: 通常エントリー
Signal 12: ❌ スキップ（4回目だから）
Signal 13, 14, 15: 通常エントリー
Signal 16: ❌ スキップ（4回目だから）
```
